...
fn test_validate_ascii() {
  let cases: &[&[u8]] = &["hello".as_bytes(), &[0xff, 0xff, 0xff]];
...
fn test_utf8_to_utf16() {
  let cases: &[&str] = &["hello", "привет", "世界", "lycoris recoil 莉可丽丝 😗"];
  ...
  for s in cases {
    ...
    {
      let output = unsafe { convert_arbitrary_utf8_to_utf16le(s.as_bytes()) };
      assert_eq!(utf16.len(), output.len());
      assert_eq!(utf16, output);
    }
